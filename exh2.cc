...
Max points found for example input:
 - Not sorting database: 1283pts in 386s
 - Sorting database: 1463pts in 0.7s (greedy approach works better!?)
*/
...
bool solution_valid() {
  if (int(players.size()) == 11) {
    return goal == 0 and def == 0 and mid == 0 and str == 0 and
...
void exhaustive_search(int k) {
  ...
  if (solution_valid() and current_points > best_points) {
    best_points = current_points;
    print_output();
    return;
  }

  // If solution not valid prune
